...
void* ds_new() {
  ...
  scal::EliminationBackoffStack<uint64_t> *ebs[g_num_threads+2];


  sds::SCVectorStack<uint64_t, scal::EliminationBackoffStack<uint64_t>> *vstack = new sds::SCVectorStack<uint64_t,
                              scal::EliminationBackoffStack<uint64_t>>(g_num_threads+2, g_threshold);



  for(uint i=0; i <= g_num_threads+1; i++) {
    ebs[i] = new scal::EliminationBackoffStack<uint64_t>(g_num_threads + 2, 
