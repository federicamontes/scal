...
void SeqAltBench::bench_func(void) {
  ...
  //Pool<sds::element<uint64_t>> *ds = static_cast<Pool<sds::element<uint64_t>>*>(data_); //for sc_distributed_stack
  uint64_t thread_id = scal::ThreadContext::get().thread_id();
  uint64_t item;
  // Do not use 0 as value, since there may be datastructures that do not
  // support it.
  
  //struct instantiation for sc_distributed_stack
